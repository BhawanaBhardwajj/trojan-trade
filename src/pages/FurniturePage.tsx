...
const FurniturePage = () => {
  const [furnitureListings, setFurnitureListings] = useState<any[]>([]);
    ...
    const fetchListings = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          setFurnitureListings(listingsWithUsers);
        } else {
          setFurnitureListings([]);
        }
      } catch (error) {
  ...
  const filteredListings = useMemo(() => {
    return furnitureListings.filter(listing => {
      // Subcategory filter
    ...
    <div className="min-h-screen bg-[hsl(var(--usc-beige))]">
      ...
      <section className="relative h-[400px] overflow-hidden">
        <div
          ...
          style={{ backgroundImage: `url(${heroFurniture})` }}
        >
        ...
        <div className="relative h-full flex items-center justify-center">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-xl text-white/95 drop-shadow-lg max-w-2xl mx-auto">
              Everything you need to make your dorm feel like home.
            </p>
          </div>
        </div>
      </section>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {/* Filters */}
        <FilterBar showVerifiedOnly categoryPage="essentials" onFilterChange={setFilters} />
          ...
          <div className="text-center py-12">
            <p className="text-muted-foreground">
              {furnitureListings.length === 0
                ? "No furniture listings available yet."
