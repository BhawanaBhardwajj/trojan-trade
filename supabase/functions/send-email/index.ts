...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    if (hookSecret) {
      ...
      console.log("[send-email] Sending OTP email to:", user.email, "action:", email_action_type);

      const emailResponse = await resend.emails.send({
        from: "Trojan Trade <no-reply@trojan-trade.com>",
        to: [user.email],
        subject:
    ...
    console.error("[send-email] Error sending verification email:", error);
        ...
        <div style="background: white; padding: 40px 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
          ...
          <div style="background: #FFF8DC; border-left: 4px solid #FFCC00; padding: 15px; margin: 20px 0; border-radius: 4px;">
            <p style="margin: 0; color: #666; font-size: 14px;">
              <strong style="color: #333;">⚠️ Security Note:</strong> This code will expire in 60 minutes. Never share this code with anyone.
            </p>
          </div>
